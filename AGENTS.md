...
A focused Nostr DM web client for encrypted direct messaging using NIP-17 gift-wrapped DMs with NIP-44 encryption. React 18 + TypeScript + Vite, styled with TailwindCSS + shadcn/ui, async state via TanStack Query, protocol via nostr-tools ^2.24.1.
...
npm run lint         # eslint . (currently unavailable: eslint is not installed)
npx vitest run src/lib/nip17/__tests__/giftwrap.test.ts  # Run a single test file
```

## Tooling and Deployment

- `.mise.toml` is the source of truth for the Node and Wrangler versions. Prefer `mise exec -- <command>` when the shell has not activated Mise.
- `.envrc` supplies Cloudflare deployment credentials. Do not read or print secrets; inspect only a specifically requested non-secret variable.
- Production is the Cloudflare Pages project `nostr-dm`, deployed from the `main` branch:

```bash
mise exec -- npm run build
direnv exec . mise exec -- wrangler pages deploy dist --project-name=nostr-dm --branch=main
```

- `.github/workflows/deploy.yml` performs the same deployment on pushes to `main` and via manual workflow dispatch.
...
All 5 auth methods must work through the unified `NostrSigner` interface:
...
4. **diVine OAuth** (`login.divine.video`) - email/password via diVine, HTTP signing
